...
\begin{frame}[label=full]{Primitive Data Types}
\small
...
\begin{frame}[label=full]{Missing Values}
\begin{itemize}
...
\begin{frame}[label=advanced]{Floating Point Numbers (IEEE 754 Standard)}
...
\begin{frame}[label=full]{Floating Point Serialization to Text}
...
\begin{frame}[label=full,fragile]{Characters (Unicode ISO/IEC 8859)}
\begin{itemize}
...
\begin{frame}[label=full]{Unicode}
\centering
...
\begin{frame}[label=full]{Dates and Times}
...
\begin{frame}[label=full]{ISO 8601 and RFC 3339 Date Format} 
\footnotesize
\renewcommand{\arraystretch}{1.25}
...
\begin{frame}[label=withcode]{Complex/Structured Data Types}
\small
\begin{block}{Python}
	\begin{itemize}
...
\begin{frame}[label=full]{Data Types in Analytics}
...
\begin{frame}[label=full,fragile]{Table Interchange/Serialization}
...
\begin{frame}[label=full]{Table Interchange/Serialization \small [cont'd]}
...
\begin{column}{0.8\textwidth}
\begin{itemize}
	\item Search the web for a CSV file of the population of all countries of the world
	\item Examine the CSV file with a text editor:
	\begin{itemize}
	...
	\end{itemize}
	\item Import the CSV file into a spreadsheet app
	\begin{itemize}
		\item Does it recognize all information correctly? If not, what is not imported well?
	\end{itemize}
	\item Export the spreadsheet as a CSV file
	\begin{itemize}
...
\begin{frame}[label=full]{Relational Databases}
...
\begin{frame}[label=full]{Relational Databases \small [cont'd]}
...
\begin{frame}[label=full]{Key-Value Data Stores}
\begin{block}{Characteristics}
\begin{itemize}
...
\begin{frame}[label=full]{Key-Value Data Stores \small [cont'd]}
\begin{block}{Advantages}
\begin{itemize}
...
\begin{frame}[label=full]{Document Databases}
...
\begin{frame}[label=full]{Document Databases \small [cont'd]}
\begin{block}{Prominent Examples}
\begin{itemize}
...
\begin{frame}[label=full]{Documents (JSON)}
...
\begin{frame}[label=full,fragile]{JSON Example -- Complex Object}
\footnotesize
...
\begin{frame}[label=full,fragile]{JSON Example -- List of Objects}
\footnotesize
\begin{minted}[bgcolor=code,frame=single,framesep=10pt]{json}
[
  {
...
\begin{frame}[label=full]{Extensible Markup Language -- XML}
\begin{itemize}
...
\begin{frame}[label=full,fragile]{XML Example}
\begin{xmlcode}
<People
...
\begin{frame}[label=full,fragile]{XML Example \small [cont'd]}
\begin{xmlcode}
...
\begin{frame}[label=full]{XML Example -- Key Points}
\small
\begin{itemize}
...
\begin{frame}[label=full,fragile]{Equivalent JSON Example}
\footnotesize
\begin{minted}[bgcolor=code,frame=single,framesep=10pt]{json}
{ "Innu": {
    "GeneralInformation": {
...
\begin{frame}[label=full,fragile]{XML --- JSON}
\begin{itemize}
...
\begin{frame}[label=full]{Property Graphs}
\begin{itemize}
...
\begin{frame}[label=full]{RDF Graphs}
\begin{itemize}
...
\begin{frame}[label=full]{Graph Queries}
\includegraphics[width=\textwidth]{screen3.png}
\scriptsize
Wang, Y., Li, Y., Fan, J., Ye, C., \& Chai, M. (2021). A survey of typical attributed graph queries. World Wide Web, 24, 297-346.
\end{frame}

\begin{frame}[label=full]{Graph Databases}
\begin{block}{Prominent Examples}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{PG-JSON Example}
\tiny
\begin{minted}[bgcolor=code,frame=single,framesep=10pt]{json}
{
  "nodes":[
...
\begin{frame}[label=withcode,fragile]{GraphSON Example}
\tiny
...
\end{minted}
\end{frame}
\begin{frame}[label=withcode,fragile]{GraphSON Example \small [cont'd]}
\tiny
\begin{minted}[bgcolor=code,frame=single,framesep=10pt,escapeinside=||]{json}
  "edges": [
    {
...
\begin{frame}[label=full,fragile]{N-Triples}
\scriptsize
...
\begin{frame}[label=full,fragile]{Turtle Example (Terse RDF Triples)}
\tiny
...
\begin{itemize}
	\item Identify information about yourself, such as names, addresses, dates, relationships (work, school, uni), etc.
	\item Draw a corresponding graph linking these pieces of information as subjects, objects, and verbs
	\item Turn the graph into a set of Terse XML Triples (''turtles'')
	\item \emph{Tip:} Do not worry about XML prefixes
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\begin{frame}[label=full,fragile]{RDF/XML}
\tiny
...
\begin{frame}[label=full]{Text Mining}
...
\begin{frame}[label=full]{Text Mining \small [cont'd]}
...
\begin{enumerate}
	...
	\item How would you use this text data?
...
\begin{frame}[label=advanced,fragile]{Regular Expressions (RegEx)}
\begin{itemize}
...
\begin{frame}[label=advanced]{Basic RegEx}
\footnotesize
...
\begin{frame}[label=advanced]{Basic RegEx Examples}
\footnotesize
\renewcommand{\arraystretch}{1.5}
...
\begin{frame}[label=advanced]{Extended RegEx}
\footnotesize
\renewcommand{\arraystretch}{1.5}
\begin{tabularx}{\textwidth}{c|X} \hline
{\bf Metacharacter} & {\bf Description} \\ \hline \hline
...
\begin{frame}[label=advanced]{Extended RegEx Examples}
\footnotesize
\renewcommand{\arraystretch}{1.5}
...
\begin{frame}[label=advanced]{Different Types of RegEx}
...
\begin{frame}[label=advanced]{Hands-On Exercises}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:Paper_and_pencil_Pinhead_icon.svg}{\includegraphics[width=.75\textwidth]{../pencilblack.png}}
\end{column}
...
\begin{frame}[label=advanced]{Levenshtein Distance}
...
\begin{frame}[label=advanced]{Hands-On Exercise}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:Paper_and_pencil_Pinhead_icon.svg}{\includegraphics[width=.75\textwidth]{../pencilblack.png}}
\end{column}
...
\begin{frame}[label=full]{Image Data}
...
\begin{frame}[label=full]{Image Analysis}
\centering
...
\begin{block}{Hands-On Exercise}
\begin{enumerate}
	\item Identify a specific business problem that can be solved by analyzing image data
	\item What image data would you need?
	\item How would you use the image data?
...
\begin{frame}[label=full]{Video Data}
...
\begin{frame}[label=full]{Video Analytics}
\begin{block}{Typical Tasks}
\begin{itemize}
	\item Object detection and object recognition
...
\begin{block}{Hands-On Exercise}
\begin{enumerate}
	\item Identify a specific business problem that can be solved by analyzing video data
	\item What video data would you need?
	\item How would you use the video data?
...
\begin{frame}[label=full]{Metadata}
\begin{itemize}
...
\begin{frame}[label=full]{Data Quality}
...
\end{column}
\begin{column}{0.8\textwidth}
Evaluate the data quality of the following information for a company that you are researching to decide whether to offer discounts on sales to this company. \\
\vspace{\baselineskip}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{l|l} 
{\bf Year} & {\bf Sales} \\ \hline 
2010 & 1,053,123 \\ \hline
2011 & 1.100,000 \\ \hline
2012 & 1,187,835 \\ \hline
2014 & 5,097,651 \\ \hline
2015 & 1,286,837 \\ \hline 
{\bf Total} & {\bf 8,573,909} \\ 
\end{tabular} \\

\vspace{\baselineskip}
The data was found on an anonymous web blog
\end{column}
\end{columns}
\end{frame}

\begin{frame}[label=full]{Data Provenance and Validity}
\centering
...
\begin{frame}[label=full]{Data Provenance and Validity \small [cont'd]}
\centering
...
\begin{frame}[label=full]{Data Cleaning}
...
\begin{frame}[label=full]{Data Validation}
\begin{itemize}
...
\begin{frame}[label=full]{Data Validation \small [cont'd]}
\begin{itemize}
