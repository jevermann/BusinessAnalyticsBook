...
\url{https://www.youtube.com/watch?v=-vqPyhyqGF4}
\end{videobox}
\end{advanced}
...
The \emph{Moving Average}\index{Moving average} model is a fundamental time series model that expresses the current value of the time series as a function of past errors or deviations, with the assumption that these errors are white noise, that is, random (Figure~\ref{fig:figure2}). An example model is given by: 
...
The \emph{Autoregressive (AR) model}\index{Autoregression} is based on the concept that current values of a series can be forecasted from previous values (Figure~\ref{fig:figure3}). An example model is:
...
A \emph{random walk with drift}\index{Random walk} adds a constant to the standard random walk, allowing the series to drift upwards or downwards over time (Figure~\ref{fig:figure4}). An example model is given by:
...
The \emph{Signal in noise}\index{Signal-in-noise} model views the time series as a combination of a true signal and random noise (Figure~\ref{fig:figure5}). An example model with a sinusoidal signal characterized by its amplitude, frequency and phase shift is:
...
\emph{Moving average smoothing}\index{Time series smoothing!Moving average} is one of the simplest and most widely used methods for smoothing time series data (Figure~\ref{fig:figure14}). It involves calculating the weighted mean of the consecutive data points within a specified window that moves along with the data:
...
Time series regression uses ordinary least squares (OLS) regression models where each series is a predictor variable. It essentially neglects the time aspect of the time series data. 

\begin{withcode}
The following R code block shows examples of time series regression to predict or explain mortality. Note the use of the \texttt{time()} function extract the timestamps from the time series data.
...
Time series regression can also use lagged time series data, that is, data of the same series that is shifted backward in time. This is somewhat similar to the autoregressive models defined below. 
\begin{withcode}
...
\noindent where the last step again assumes weak stationarity. See Figure~\ref{fig:figure8a} for an example of ACF at different lags.
...
If the sample ACF of $n$ values of a time series for a given lag exceeds the lower or upper bounds of the confidence interval, the ACF is statistically significantly different from $0$, and the time series is unlikely to be white noise (Figure~\ref{fig:figure7a}).
...
The theoretical ACF of a given AR(p) model can be calculated analytically. 
\begin{withcode}
In R, the \texttt{ARMAacf()} function can be used for this by spacifying the autoregressive coefficients $\phi$. The following R code block simulates 200 observations of an AR(2) time series and plots the simulated (blue) versus theoretical (red) ACF values, shown in Figure~\ref{fig:figure18}. 
\end{withcode}
The theoretical values can be used to determine whether a specific time series conforms to a particular AR(p) model. The ACF of an AR(p) model is characterized by a slow decline of its values past a lag of $p$, as shown in Figure~\ref{fig:figure18}.
...
The theoretical ACF of a given MA(q) model can be calculated analytically. 
\begin{withcode}
In R, the \texttt{ARMAacf()} function can be used for this, by specifying the moving average coefficients $\theta$. Similar to the previous example, the following R code simulates 200 observations of an MA(2) model and plots the simulated (blue) versus theoretical (red) ACF values, shown in Figure~\ref{fig:figure19}.
\end{withcode} 
In contrast to an AR(2) model, the ACF does not gradually diminish, but becomes 0 after lag $q$. The simulated values in Figure~\ref{fig:figure19} confirm this as they are largely stastistically non-significant past a lag of 2.
...
Identifying the initial ARMA order $p$ and $q$ should be done based on the ACF and PACF functions, using Table~\ref{tab:modelselection} as a basis. Often, multiple model may need to be tried, altering the AR and MA orders $p$ and $q$ in small steps (Figure~\ref{fig:figure22}). 
...
Once the analyst has selected the final model and is satisfied that it fits well, future values of the time series can be forecasted from the fitted model. An important property of ARIMA predictions is that they quickly settle to the mean, with a constant prediction error, reflecting the stationarity of the differenced and transformed time series (Figure~\ref{fig:figure21}).
...
\begin{withcode}
As an example, consider the US gross national product quarterly time seriess. An initial AR(1) model shows that the squared residuals have some dependence. This dependence can be accounted for by explicitly modeling the residuals as ARCH(1). The \texttt{fGarch} library for R provides the \texttt{garchFit()} function to these kinds of models. The squared residuals of the final model are shown in Figure~\ref{fig:figure35}.
