...
\begin{frame}[label=full]{Based On}
\small
\begin{block}{}
...
\begin{frame}[label=full]{Time Series Application Examples}
\begin{itemize}
...
\begin{frame}[label=full]{Characteristics of Time Series}
...
\begin{frame}[label=full]{Time Series Statistical Models}
\begin{itemize}
...
\begin{frame}[label=full]{Moving Average}
Example:
\begin{align*}v_t = \frac{1}{3} ( w_{t} + w_{t-1} + w_{t-2}) \qquad \text{(Sum of coefficients equals 1)}
\end{align*}
\centering

\includegraphics[width=.55\textwidth]{figure2.pdf}
\end{frame}

\begin{frame}[label=withcode,fragile]{Moving Average in R}
\begin{Rcode}
# Load ASTSA library
library(astsa)
...
\begin{frame}[label=full]{Autoregression}
Example:
\begin{align*}x_t = x_{t-1} - 0.9 x_{t-2} + w_t \qquad \text{(Coefficients less or equal 1)}
\end{align*}
\centering
...
\begin{frame}[label=full,fragile]{Hands-On Exercise}
\begin{columns}
...
Use the following white noise (''error'') data to construct the elements of the time series:

\begin{align*}x_t = x_{t-1} - 0.9 x_{t-2} + w_t \end{align*}

0.819, .088, -0.171, 1.302, 0.888, -1.160, -1.345, 0.195, 0.226, 2.275
\end{column}
\end{columns}
\end{frame}


\begin{frame}[label=withcode,fragile]{Autoregression in R}
\begin{Rcode}
...
\begin{frame}[label=full]{Random Walk with Drift}
Example:
\begin{align*}x_t &= \delta + x_{t-1} + w_t = \delta t + \sum_{j=1}^t w_j \qquad \text{(Coefficient of $x_{t-1}$ is 1)}
\end{align*}

\includegraphics[width=\textwidth]{figure4.pdf}
\end{frame}

\begin{frame}[label=withcode,fragile]{Random Walk with Drift in R}
\begin{Rcode}
# White noise
w = rnorm(200)
# Random walk
...
\begin{frame}[label=full]{Signal in Noise}
\begin{columns}
...
\begin{frame}[label=withcode,fragile]{Signal in Noise in R}
\begin{Rcode}
# Create signal
...
\begin{frame}[label=withcode,fragile]{Basic Time Series Operations in R}
...
\begin{frame}[label=full]{Smoothing a Time Series}
\begin{itemize}
   \item Moving average
...
\begin{frame}[label=full,fragile]{Moving Average}
...
\begin{column}{0.8\textwidth}
\begin{enumerate}
\item Use the Moving Average Smoothing on the following time series data of company profits (in millions of dollars) with $a_{j-1} = a_j = a_{j+1} = 1/3$: \\

\vspace{\baselineskip}
-0.052, -1.788,  0.609,  1.824,  0.101,  1.443,  1.210, -0.916,  0.278, -0.211 \\

\vspace{\baselineskip}

\item Sketch the original and the smoothed time series
\end{enumerate}
\end{column}
\end{columns}
\end{frame}

\begin{frame}[label=full,fragile]{Kernel Smoothing}
...
\begin{frame}[label=full,fragile]{Locally Weighted Regression}
\begin{itemize}
...
\begin{frame}[label=full,fragile]{Smoothing Splines}
\begin{itemize}
...
\begin{enumerate}
  ...
  \item How does the moving average change each series?
\end{enumerate}
\hrule
\footnotesize
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{Time Series Regression Example}
...
\begin{frame}[label=withcode,fragile]{Time Series Regression Example \small [cont'd]}
...
\begin{frame}[label=withcode,fragile]{Time Series Regression Example}{With Lagged Variables}
...
\begin{frame}[label=full]{Strict Stationarity}
...
\begin{frame}[label=full]{Measures of Dependence}
...
\begin{frame}[label=full]{Measures of Dependence \small [cont'd]}
...
\begin{frame}[label=full]{Autocorrelation Function (ACF) at Different Lags}
\centering
\includegraphics[height=3.25in]{figure8a.pdf}
\end{frame}


\begin{frame}[label=withcode,fragile]{Autocorrelation Function (ACF) in R}
...
\begin{frame}[label=full]{Weak Stationarity}
...
%\begin{frame}[label=full,fragile]{Standard Deviations of ACF and CCF}
%\small
%\begin{enumerate}
...
\begin{frame}[label=full]{Autocorrelation Function (ACF) of White Noise}
\begin{center}
\includegraphics[width=\textwidth]{figure7a.pdf}
\end{center}
\end{frame}

\begin{frame}[label=withcode,fragile]{Autocorrelation Function (ACF) of White Noise in R}
\begin{Rcode}
library(astsa)
t <- ts(rnorm(500))
...
%\begin{frame}[label=full,fragile]{Cross-Correlation Function}
...
%\begin{frame}[label=full]{Cross-Correlation Function}
%\centering

%\includegraphics[width=.7\textwidth]{figure9.pdf}
%\end{frame}

%\begin{frame}[label=full,fragile]{Pre-Whitening}
...
%\begin{frame}[label=full]{Pre-Whitening}
%\centering

%\includegraphics[height=3in]{figure10.pdf}
%\end{frame}

\begin{frame}[label=full]{Dealing with Non-Stationarity}
...
\begin{frame}[label=full]{Dealing with Non-Stationarity -- Detrending}
Assume $x_t = \mu_t + y_t$ where $\mu_t$ is the trend and $y_t$ is stationary.
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{Detrending in R}
\begin{Rcode}
...
\begin{column}{0.8\textwidth}
\begin{enumerate}
\item Calculate the first and second differences for the following time series data of quarterly company sales (in millions of dollars):\\

\vspace{\baselineskip}
2.937,  1.515,  6.513, 10.984, 15.170, 21.143, 27.979, 34.566, 43.591, 53.414 \\

\vspace{\baselineskip}
\item Sketch the original time series, the first difference, and the second difference
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=withcode,fragile]{Differencing in R}
\begin{Rcode}
set.seed(42)
...
\begin{frame}[label=full]{Differencing}
\begin{center}
\includegraphics[height=3in]{figure40a.pdf}
\end{center}
\end{frame}

\begin{frame}[label=withcode,fragile]{Detrending and Differencing in R}
...
\begin{frame}[label=full]{ACF after Detrending and Differencing}
\centering
\includegraphics[width=.9\textwidth]{figure41.pdf}
\end{frame}

%\begin{frame}[label=full]{Detrending and Differencing in R}
%\centering
...
\begin{frame}[label=full]{ARIMA Models}
\begin{itemize}
...
\begin{frame}[label=full]{Differencing \small [cont'd]}
...
\begin{frame}[label=full]{AR(p) Models}
...
%\begin{frame}[label=full]{AR(1) Models}
...
\begin{frame}[label=withcode,fragile]{AR(p) Models -- Theoretical ACF and Simulations}
...
%\begin{frame}[label=full]{AR Fitting Methods}
%\begin{block}{OLS}
%\begin{itemize}
...
\begin{frame}[label=full]{MA(p) Models}
...
%\begin{frame}[label=full]{MA(1) Models}
...
\begin{frame}[label=withcode,fragile]{MA(p) Models -- Theoretical ACF and Simulations}
...
\begin{frame}[label=full]{ARMA(p, q) Models}
...
%\begin{frame}[label=full,fragile]{Parameter Redundancy -- Example 1}
%\begin{align*}
...
%\begin{frame}[label=full,fragile]{Parameter Redundancy -- Example 2}
%\begin{align*}
...
%\begin{frame}[label=full,fragile]{Equivalent Models}
...
\begin{frame}[label=full,fragile]{Partial Autocorrelation Function (PACF)}
...
\begin{frame}[label=withcode,fragile]{Theoretical Partial Autocorrelation Function (PACF)}
\begin{Rcode}
...
\begin{frame}[label=full]{ARMA(p, q) Models -- Model Selection}
\small
...
\end{column}
\begin{column}{0.8\textwidth}
Sketch the shape of the ACF and PACF functions for lags up to 10 that you expect of the following ARMA models:\\
...
\begin{frame}[label=advanced]{ARIMA(p, d, q) Models}
...
\begin{frame}[label=full]{Building ARIMA Models}
\begin{enumerate}
  \item Plot the data
...
\begin{frame}[label=full]{Identifying Dependence Order}
\small
...
\begin{frame}[label=withcode,fragile]{Example}
...
\begin{frame}[label=full]{Example}
\begin{columns}
...
\begin{frame}[label=full,fragile]{Mis-Fit Example}
\begin{Rcode}
...
\begin{frame}[label=full,fragile]{Example \small [cont'd]}
...
\begin{frame}[label=withcode,fragile]{Example \small [cont'd]}
\begin{textcode}
> sarima(gnpgr, 1, 0, 0)
...
\begin{frame}[label=full,fragile]{Example -- Forecasting}
\begin{Rcode}
...
%\begin{frame}[label=full]{Pure Seasonal ARMA Models}
%\begin{align*}
...
%\begin{frame}[label=full]{Pure Seasonal ARMA Models -- ACF and PACF}
...
%\begin{frame}[label=full]{Mixed Seasonal ARIMA Models SARIMA(p,d,q)x(P,D,Q)\textsubscript{s}}
...
%\begin{frame}[label=full,fragile]{Example SARIMA(0,1,1)x(0,1,1)\textsubscript{12} Model}
%\begin{align*}
...
%\begin{frame}[label=full,fragile]{Pure Seasonal AR(1) Example}
%\begin{align*}
...
%\begin{frame}[label=full,fragile]{Pure Seasonal AR(1) Example \small [cont'd]}
%\begin{center}
...
%\begin{frame}[label=full,fragile]{Example SARIMA(0,1)x(1,0)\textsubscript{12} Model \small [cont'd]}
%\begin{align*}
...
%\begin{frame}[label=full]{Example SARIMA(0,1)x(1,0)\textsubscript{12} Model \small [cont'd]}
%\centering
%\includegraphics[width=\textwidth]{figure30.pdf}
%\end{frame}


%\begin{frame}[label=full,fragile]{Example SARIMA}
...
%\begin{frame}[label=full]{Example SARIMA \small [cont'd]}
%\centering
%\includegraphics[height=3in]{figure31.pdf}
%\end{frame}

%\begin{frame}[label=full,fragile]{Example SARIMA \small [cont'd]}
%Sample ACF and PACF
%\begin{Rcode}
...
\begin{frame}[label=advanced]{General Autoregressive Conditional Heteroscedastic (GARCH) Models}
...
\begin{frame}[label=advanced]{Example: Combined AR(1) and ARCH(1) Model}
...
Example: US GNP Data
\centering
\includegraphics[width=.65\textwidth]{figure35.pdf} \\

Squared residuals may have some dependence.
\end{frame}

\begin{frame}[label=advanced,fragile]{Example: Combined AR(1) and ARCH(1) Model}
Examine residuals:
\begin{Rcode}
...
\begin{frame}[label=advanced]{ARCH Extensions}
...
\begin{frame}[label=withcode,fragile]{GARCH Example -- DJIA Returns}
\begin{Rcode}
library(fGarch)
# Log transform
...
\begin{frame}[label=advanced]{ARCH Example -- DJIA Returns}{ACF}
\centering
\includegraphics[height=1.5in]{figure36.pdf}
\includegraphics[height=1.5in]{figure37.pdf}
\end{frame}

\begin{frame}[label=withcode,fragile]{GARCH Example -- DJIA Returns}{Results}
\begin{textcode}
...
\begin{frame}[label=withcode]{Appendix -- Basic Time Series Functions in R}
\footnotesize
...
\begin{frame}[label=withcode]{Appendix -- Time Series Functions in the \texttt{astsa} library}
\footnotesize
\renewcommand{\arraystretch}{1.5}
\begin{tabularx}{\textwidth}{l|X} \hline
