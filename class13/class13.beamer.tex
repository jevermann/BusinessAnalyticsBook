...
\begin{frame}[label=full]{Principal Components Analysis (PCA)}
\begin{itemize}
...
\begin{frame}[label=full]{PCA \small [cont'd]}
\begin{columns}
...
\begin{frame}[label=full]{PCA -- Example and Biplot}
\begin{columns}
...
\begin{frame}[label=full]{PCA -- Technicalities}
\begin{itemize}
...
\begin{frame}[label=full]{PCA -- Screeplot \small [cont'd]}
\begin{block}{Choosing the Number of Principal Components}
\small
\begin{itemize}
   \item Eigenvalue $\lambda > 1$
   \item Eigenvalue $>$ random data eigenvalue (''parallel analysis'')
...
\begin{frame}[label=full,fragile]{Hands-On Exercise -- Principal Components Analysis}
\begin{columns}
...
A dataset with 14 variables (Holzinger Swineford 1939) was input to a principal component analysis. The results are reported as follows:

\small
\begin{textcode}
Standard deviations:
     Comp.1      Comp.2      Comp.3 
105.4591293   3.4644213   2.0153651 
     Comp.4      Comp.5      Comp.6
  1.4649669   1.3245455   0.9590910 
     Comp.7      Comp.8      Comp.9 
  0.9163258   0.8293599   0.7707158
    Comp.10     Comp.11     Comp.12 
  0.7110467   0.6031490   0.5891680 
    Comp.13     Comp.14 
  0.4592334   0.3143720 
\end{textcode}
\normalsize

Sketch the screeplot, find the cumulative variance explained for each component, and explain how many components you would retain.
\end{column}
\end{columns}
\end{frame}


\begin{frame}[label=withcode,fragile]{PCA in R}
...
\begin{frame}[label=withcode]{PCA in R \small [cont'd]}
...
\begin{frame}[label=withcode,fragile]{PCA in R \small [cont'd]}
continued \ldots
\begin{Rcode}
...
\begin{frame}[label=withcode]{Hands-On Exercises -- PCA}
\begin{columns}
...
\begin{frame}[label=full]{Clustering}
\begin{block}{Goals}
\begin{itemize}
...
\begin{frame}[label=full]{K-Means Clustering}
\begin{itemize}
...
\begin{frame}[label=full]{K-Means Clustering -- Iterative Cluster Assignment}
\begin{columns}
...
\begin{frame}[label=full]{K-Means Clustering -- Randomized Starting}
\begin{columns}
\begin{column}{0.35\textwidth}
\begin{itemize}
...
\begin{frame}[label=full,fragile]{Hands-On Exercise -- K-Means Clustering}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:Paper_and_pencil_Pinhead_icon.svg}{\includegraphics[width=.75\textwidth]{../pencilblack.png}}
\end{column}
\begin{column}{0.8\textwidth}
Manually cluster the following three-dimensional dataset into three clusters. Show all work.

\bigskip
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{c|c|c} \hline
{\bf Income} & {\bf Spending} & {\bf Debt} \\ \hline
1 & 1 & 1 \\
2 & -1 & 1 \\
1 & 3 & 3 \\
2 & 4 & 4 \\
...
\vspace{\baselineskip}
\begin{itemize}
   \item Use Euclidean distances $d = \sqrt{(x_i - x_m)^2 + (y_i - y_m)^2 + (z_i - z_m)^2}$.
   \item You do not need to standardize the data
\end{itemize}
\end{column}
\end{columns}
\end{frame}

% Data for exercise generated using the following R script
%> library(MASS)
%> mu <- c(2, 2, 2)
%> sigma <- diag(1, nrow=3)
%> cluster1 <- mvrnorm(n=3, mu, sigma)
%> mu <- c(2, 4, 4)
%> cluster2 <- mvrnorm(n=3, mu, sigma)
%> mu <- c(4, 4, 6)
%> cluster3 <- mvrnorm(n=3, mu, sigma)
%> cluster3 <- round(cluster3)
%> cluster2 <- round(cluster2)
%> cluster1 <- round(cluster1)
%> d <- rbind(cluster1, cluster2, cluster3)

\begin{frame}[label=withcode,fragile]{K-Means Clustering in R}
Simulated example:
\begin{Rcode}
...
\begin{frame}[label=withcode,fragile]{K-Means Clustering in R \small [cont'd]}
continued \ldots
\begin{Rcode}
...
\begin{frame}[label=withcode]{K-Means Clustering in R \small [cont'd]}
\centering

\includegraphics[height=3in]{kmeans.pdf}
\end{frame}

\begin{frame}[label=withcode]{Hands-On Exercises -- K-Means Clustering}
\begin{columns}
\begin{column}{0.1\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}
\end{column}
...
\begin{frame}[label=full]{Hierarchical Clustering}
...
\begin{frame}[label=full]{Hiearchical Clustering}
...
\begin{frame}[label=full]{Hierarchical Clustering -- Common Distance Metrics}
...
\begin{frame}[label=full]{Hierarchical Clustering -- Common Linkage Criteria}
...
\end{frame}
\begin{frame}[label=full]{Hierarchical Clustering -- How Many Clusters?}
\begin{itemize}
...
\begin{frame}[label=full,fragile]{Hands-On Exercise -- Hierarchical Clustering}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:Paper_and_pencil_Pinhead_icon.svg}{\includegraphics[width=.75\textwidth]{../pencilblack.png}}
\end{column}
\begin{column}{0.8\textwidth}
Use agglomerative hierarchical clustering on the following three-dimensional dataset. Show all work.
...
\begin{frame}[label=withcode,fragile]{Hierarchical Clustering in R}
\begin{Rcode}
...
\begin{frame}[label=withcode,fragile]{Hierarchical Clustering in R \small [cont'd]}
continued \ldots
\begin{Rcode}
...
\begin{frame}[label=withcode]{Hierarchical Clustering in R \small [cont'd]}
\centering
...
\begin{frame}[label=withcode]{Hands-On Exercises -- Hierarchical Clustering}
\begin{columns}
\begin{column}{0.1\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}
\end{column}
