...
\begin{frame}[label=full]{Why Visualize?}
...
\begin{frame}[label=full]{Hands-On Exercises}
\begin{columns}
...
For each scenario, identify whether it is an example of visual Discovery, declarative Visualization, or operational Visualization.

\begin{enumerate}
\item A scientist explores climate data looking for unknown patterns.
\item A CEO presents quarterly financial results to investors.
\item A network administrator monitors server activity in real time.
\item A marketing analyst experiments with different charts while exploring data.
\item A dashboard displays factory temperatures every second.
\end{enumerate}
\end{column}
\end{columns}
\end{frame}

\begin{frame}[label=full]{Purpose of Visualization}
\begin{itemize}
...
\begin{frame}[label=full]{Quantitative Messages}
\begin{enumerate}
...
\end{column}
\begin{column}{0.8\textwidth}
For each scenario, choose the most appropriate type of plot and explain your choice.

\begin{itemize}
\item Monthly sales over five years
\item Market share of five companies
\item Distribution of employee salaries
\item Relationship between advertising spending and sales
\item Customer satisfaction by department
\item Number of accidents by weekday
\item Population density across provinces
\end{itemize}

Possible plot types to consider are bar charts, line chart, pie chart, histogram, scatterplot, box plot, maps
\end{column}
\end{columns}
\end{frame}

\begin{frame}[label=full]{Plot Elements}
...
\end{column}
\begin{column}{0.8\textwidth}
Suppose you are visualizing information about cities.
...
Assign each variable to a visual property and explain your choices.
\end{column}
\end{columns}
\end{frame}
...
\begin{enumerate}
\item Which visualization would you choose?
\item Sketch the visualization.
\item What conclusion would you draw?
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\end{column}
\begin{column}{0.8\textwidth}
A local coffee shop wants to present the following information:

\begin{itemize}
\item Monthly revenue
\item Customer age groups
\item Drinks sold by category
\item Busiest hour of the day
...
\begin{enumerate}
\item Choose an appropriate type of visualization.
\item Explain why it is appropriate.
\item Sketch the visualization.
\end{enumerate}
\end{column}
\end{columns}
...
\begin{frame}[label=full]{Honesty in Visualization}
...
\begin{frame}[label=full]{Dark Patterns -- Omitted Data}
\centering
...
\begin{frame}[label=full]{Dark Patterns -- 3D Pie Charts}
\centering
...
\begin{frame}[label=full]{Dark Patterns -- Comparing Pie Charts}
\centering
...
\begin{frame}[label=full]{Dark Patterns -- Scaling Axes and Aspect Ratios}
\centering
...
\begin{frame}[label=full]{Dark Patterns -- Scaling Multiple Dimensions}
\centering
...
\begin{frame}[label=advanced]{Special Types of Data and Visual Analytics}
	\begin{itemize}
		\item Streaming data
		\begin{itemize}
...
\begin{frame}[label=full]{Colour Palettes}
\begin{itemize}
...
\begin{frame}[label=full]{Types of Colour Palettes}
...
\begin{frame}[label=full]{Types of Colour Palettes \small [cont'd]}
...
\begin{frame}[label=full]{CVD (Colour Vision Deficiency)}
\begin{itemize}
  \item Monochromatism
...
\begin{frame}[label=full]{Original}
...
\begin{frame}[label=full]{Simulated Colour Vision Deficiencies}
\begin{tabular}{cc} 
...
\begin{frame}[label=full]{Example: Colourbrewer Palette ''Paired''}
  \includegraphics[width=\textwidth]{brewer.paired.cvd.pdf}
\end{frame}

\begin{frame}[label=full]{Viridis Colour Palette}
...
\end{column}
\begin{column}{0.8\textwidth}
A graph distinguishes two categories using only red and green.

\begin{enumerate}
\item Why could this be problematic?
\item Suggest two alternative color choices.
\item Suggest one additional visual feature that could distinguish the categories without relying only on color.
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\end{column}
\begin{column}{0.8\textwidth}
For each situation, choose the most appropriate color palette from among sequential, diverging, and spectral palettes.

\begin{enumerate}
\item Temperature above or below average
\item Election winners by political party
\item Income from lowest to highest
\item Risk levels (Low--Medium--High)
\item Species of birds
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\end{column}
\begin{column}{0.8\textwidth}
Consider the following description of a visualization:

\begin{quote}
''A rainbow-colored three-dimensional pie chart with 18 slices compares monthly expenses.''
\end{quote}

List at least five changes that would improve this visualization.
\end{column}
\end{columns}
\end{frame}


%\begin{frame}[label=full]{Plots for One Variable}
...
%\begin{frame}[label=full]{Plots for Two Variables}
...
%\begin{frame}[label=full]{Plots for Two Variables, cont'd}
...
%\begin{frame}[label=full]{Plots for Three Variables}
	%\begin{block}{Continuous}
		%\begin{itemize}
...
%\begin{frame}[label=full]{Visualizing Errors and Uncertainty}
%\begin{block}{Purpose}
	%\begin{itemize}
...
\begin{frame}[label=withcode]{Popular Graphics Libraries and Frameworks}
\begin{block}{R}
\begin{itemize}
...
\begin{frame}[label=withcode]{Example Dataset}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{Example Dataset -- Read Data}
\begin{Rcode}
...
\begin{frame}[label=withcode,fragile]{Load Graphics Libraries}
\small
\begin{Rcode}
library(ggplot2)
library(ggpattern)
...
\begin{frame}[label=withcode,fragile]{Histogram}
...
\begin{frame}[label=withcode]{Histogram}
\begin{columns}
...
\begin{frame}[label=withcode,fragile]{Density Plot}
...
\begin{frame}[label=withcode,fragile]{Combining Geoms}
\begin{Rcode}
...
\begin{frame}[label=withcode,fragile]{Labelling}
\begin{Rcode}
...
\begin{frame}[label=withcode,fragile]{Area Plot}
\footnotesize
\begin{Rcode}
data %>% group_by(Year) %>%
...
\begin{frame}[label=withcode,fragile]{Column Chart}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{Column Chart \small [cont'd]}
\begin{Rcode}
col.data |> 
...
\begin{frame}[label=withcode,fragile]{Column Chart with Patterns}
\begin{Rcode}
...
\begin{frame}[label=withcode,fragile]{Box Plot}
...
\begin{frame}[label=withcode]{Boxplot (XKCD)}
 \includegraphics[width=\textwidth]{xkcd_box_plot.png}
\end{frame}

\begin{frame}[label=withcode,fragile]{Violin Plot}
\begin{Rcode}
data |>
  pivot_longer(cols=c('City', 'Hwy'), 
               names_to='metric', 
...
%\begin{frame}[label=withcode,fragile]{Dot Plot}
%\footnotesize
%\begin{Rcode}
%data %>% 
...
%\begin{frame}[label=withcode]{Dot Plot}
  %\includegraphics[width=\textwidth]{fuel.dotplot.pdf}
%\end{frame}

%\begin{frame}[label=withcode,fragile]{Dot Plot (with Violin and Range Summary)}
%\footnotesize
%\begin{Rcode}
%data %>% 
  %filter(Year > 2019) %>%
...
%\begin{frame}[label=withcode]{Dot Plot (with Violin and Range Summary)}
  %\includegraphics[width=\textwidth]{fuel.dotplotjviolinsummary.pdf}
%\end{frame}

\begin{frame}[label=withcode,fragile]{Count Plot}
\begin{Rcode}
data %>% 
...
\begin{frame}[label=withcode,fragile]{Jitter Plot}
\begin{Rcode}
data |> 
  filter(Year >= 2020) |>
...
\begin{frame}[label=withcode,fragile]{Points Plot}
\begin{Rcode}
data |> 
...
\begin{frame}[label=withcode,fragile]{Lines and Points Plot}
\begin{Rcode}
data |> 
...
\begin{frame}[label=withcode,fragile]{Pie Chart}
\begin{columns}
\begin{column}{.55\textwidth}
\begin{Rcode}
data |>
...
\begin{frame}[label=withcode]{Pie Charts (XKCD)}
  \includegraphics[width=\textwidth]{xkcd_pie_charts.png}
\end{frame}

%\begin{frame}[label=full,fragile]{Donut Chart}
%\footnotesize
%\begin{minted}{R}
%holesize <- 2
...
%\begin{frame}[label=full]{Donut Chart}
%\centering
  %\includegraphics[width=.8\textwidth]{fuel.donut.pdf}
%\end{frame}

\begin{frame}[label=withcode,fragile]{Radar Plot}
Prepare data:
\begin{Rcode}
radardata <- data |>
...
\begin{frame}[label=withcode,fragile]{Lines with Multiple Axes}
\footnotesize
\begin{Rcode}
data |>
   group_by(Year) |>
...
\begin{frame}[label=withcode]{Lines with Multiple Axes}
\begin{columns}
\begin{column}{.75\textwidth}
...
\begin{frame}[label=withcode,fragile]{Local Regression Smoothing Plot}
\footnotesize
\begin{Rcode}
data |>
  ggplot(aes(City, Hwy)) +
...
%\begin{frame}[label=withcode,fragile]{Local Regression Smoothing Plot (with error bars)}
%\footnotesize
%\begin{Rcode}
%data %>% 
...
%\begin{frame}[label=withcode]{Local Regression Smoothing Plot (with error bars)}
  %\includegraphics[width=\textwidth]{fuel.linesSmoothErrorBar.pdf}
%\end{frame}

%\begin{frame}[label=full,fragile]{Local Regression Smoothing Plot (with cross bars)}
%\footnotesize
%\begin{Rcode}
%data %>% 
  %ggplot(aes(Year, Range)) +
...
%\begin{frame}[label=withcode]{Local Regression Smoothing Plot (with cross bars)}
  %\includegraphics[width=\textwidth]{fuel.linesSmoothCrossBar.pdf}
%\end{frame}

\begin{frame}[label=withcode,fragile]{2D Density Plot}
\begin{Rcode}
data |>
...
\begin{frame}[label=withcode,fragile]{2D Bin Plot}
\begin{Rcode}
data %>% 
  ggplot(aes(x=Hwy, y=City)) + 
...
\begin{frame}[label=withcode,fragile]{3D Raster/Tile Plot}
\footnotesize
\begin{Rcode}
data |>
...
\begin{frame}[label=withcode,fragile]{Complex Example -- 3D Raster Plot with Rug}
\begin{Rcode}
data |>
  ggplot(aes(x=Hwy, y=City, fill=Range)) + 
    geom_tile() + 
