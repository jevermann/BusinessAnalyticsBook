...
\url{https://www.youtube.com/watch?v=sOA0MirVq_w}
\end{videobox}
\end{advanced}
...
\begin{exerciseboxpaper}
For each situation explain why the visualization is misleading and describe how you would improve it.

\begin{enumerate}
\item A bar chart starts its vertical axis at 95 instead of 0.
\item A pie chart compares the sales of twelve different products.
\item A graph uses large three-dimensional bars.
\item Only data from one unusually good month is shown.
\item Two variables are plotted using differently scaled vertical axes so that their lines appear highly correlated.
\end{enumerate}
\end{exerciseboxpaper}
...
\begin{exerciseboxpaper}
Sketch an appropriate visualization for each dataset.
...
\[
(2,3),\ (3,5),\ (4,4),\ (5,7),\ (6,8),\ (7,9)
\]

Then answer:

\begin{enumerate}
\item Which graph best shows a trend?
\item Which graph best compares categories?
\item Which graph best shows a relationship?
\end{enumerate}
\end{exerciseboxpaper}
...
\begin{exerciseboxpaper}
Imagine yourself in each of the following roles. For each role, describe at least three related charts/plots on your dashboard and the type of plot that you would choose. 

\begin{enumerate}
\item Machine controller for a CNC mill
\item Warehouse manager in a logistics company
\item Marketing vice-president for Canada
