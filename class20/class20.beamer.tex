...
\begin{frame}[label=full]{Further Reading}
\begin{block}{}
...
\begin{frame}[label=full]{Big Data}
...
\begin{frame}[label=full]{Big Data Example -- CERN}
...
\begin{frame}[label=full]{Distributed Data and Distributed Computation}
\begin{block}{Hadoop}
\begin{itemize}
...
\begin{frame}[label=full]{Hadoop Benefits}
\begin{itemize}
...
\begin{frame}[label=full]{Main Hadoop Components}
\begin{itemize}
...
\begin{frame}[label=full]{HDFS Principles}
\begin{itemize}
...
\begin{frame}[label=full]{HDFS Architecture}
\centering
...
%\begin{frame}[label=full]{HDFS Replication}
%\begin{itemize}
...
%\begin{frame}[label=full]{HDFS Storage Policies}
%\renewcommand{\arraystretch}{1.5}
...
\begin{frame}[label=withcode]{Working with HDFS}
...
\begin{frame}[label=withcode]{Working with HDFS \small [cont'd]}
Continued \ldots \\
...
\begin{frame}[label=withcode,fragile]{Working with HDFS -- Examples}
...
\begin{frame}[label=withcode,fragile]{Working with HDFS -- Examples \small [cont'd]}
...
\begin{frame}[label=withcode]{Hands-On Exercise -- HDFS} 
\begin{columns}
...
\begin{frame}[label=full]{MapReduce}
...
\begin{frame}[label=full]{MapReduce -- Basic Steps}
\begin{enumerate}
\item Map
\begin{itemize}
...
\begin{frame}[label=full]{Hands-On Exercise --- Map-Reduce}
\begin{columns}
\begin{column}{0.2\textwidth}
...
A company's sales data are stored in each branch. They use Map-Reduce to find totals by product category. \\
\bigskip
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|c|l|l|r|}
\hline
\textbf{Transaction} & \textbf{Store} & \textbf{Category} & \textbf{Amount (\$)} \\
\hline
1 & Store\_A & Electronics & 500 \\
2 & Store\_B & Clothing    & 200 \\
3 & Store\_A & Electronics & 300 \\
4 & Store\_C & Grocery     & 150 \\
5 & Store\_B & Clothing    & 400 \\
\hline
\end{tabular}
\end{column}
\end{columns}
...
\bigskip
\footnotesize
Define Map and Reduce phases to calculate how much revenue each product category generated (across all stores). 

\smallskip

\begin{enumerate}
   \item What are the inputs and the output keys and values of the Map phase? 
   \item What are the keys and values of the output of the Shuffle phase?
   \item What does the Reduce phase do? What are the keys and values of its output? How many Reduce instances are there?
\end{enumerate}
\end{frame}

\begin{frame}[label=advanced]{MapReduce -- Optional Functions}
...
\begin{frame}[label=advanced]{MapReduce -- Optional Functions \small [cont'd]}
...
\begin{frame}[label=full]{MapReduce on Hadoop -- YARN cluster manager}
\begin{itemize}
...
\begin{frame}[label=withcode]{MapReduce on Hadoop}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{MapReduce Example -- Word Count}
...
\begin{frame}[label=withcode,fragile]{MapReduce Example -- Word Count \small [cont'd]}
The Reducer:
\begin{pythoncode}
#!/usr/bin/env python
import sys
...
\begin{frame}[label=withcode]{Hands-On Exercise -- Map Reduce}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}
\end{column}
...
\begin{frame}[label=full]{MapReduce Use Case -- Scalable Process Discovery}
\begin{columns}
...
\begin{frame}[label=full]{Example -- Flexible Heuristic Miner MapReduce Pipeline}
...
\begin{frame}[label=full]{MapReduce Use Case -- Results}
\begin{center}
...
\begin{frame}[label=full]{Apache Pig}
\begin{columns}
\begin{column}{.3\textwidth}
\centering
...
\begin{frame}[label=full,fragile]{Apache Pig Latin Example -- Word Count}
\begin{sqlcode}
...
\begin{frame}[label=full]{Apache Pig Latin -- Relational Operators}
\renewcommand{\arraystretch}{1.5}
...
\begin{frame}[label=full]{Apache Hive}
\begin{columns}
...
\begin{frame}[label=full,fragile]{HiveQL Example -- Word Count}
\begin{sqlcode}
...
\begin{frame}[label=advanced]{Apache Spark}
\begin{columns}
\begin{column}{.4\textwidth}
...
\begin{frame}[label=advanced]{Apache Spark -- Characteristics}
\small
\begin{itemize}
...
\begin{frame}[label=advanced]{Apache spark -- Main Components}
\centering
...
\begin{frame}[label=advanced]{Apache Spark -- Cluster Overview}
\begin{center}
...
\begin{frame}[label=advanced]{Apache Spark -- Spark on Hadoop}
\begin{itemize}
...
\begin{frame}[label=advanced]{Apache Spark -- Data Abstractions}
\small
\begin{enumerate}
...
\begin{frame}[label=advanced]{Apache Spark -- Execution Principles}
...
\begin{frame}[label=withcode,fragile]{Apache Spark -- Basics}
...
%\begin{frame}[label=full,fragile]{Apache Spark -- Basics \small [cont'd]}
...
\begin{frame}[label=withcode,fragile]{Apache Spark -- Basics \small [cont'd]}
...
\begin{frame}[label=advanced]{Apache Spark -- Basic Transformations and Actions}
...
\begin{frame}[label=withcode,fragile]{Apache Spark -- Schemas and DataFrames}
...
\begin{frame}[label=withcode,fragile]{Apache Spark -- SQL}
\small
...
\begin{frame}[label=withcode,fragile]{Apache Spark -- Compare to PostgreSQL}
\begin{bashcode}
psql
\end{bashcode}
...
\begin{frame}[label=advanced]{Apache Spark -- ML}
...
\begin{frame}[label=advanced]{Apache Spark -- ML Pipelines}
\begin{itemize}
...
\begin{frame}[label=advanced]{Apache Spark -- Selection of available ML Models}
...
%\begin{frame}[label=full,fragile]{Apache Spark -- ML Pipelines}
%Get the dataset:
%\begin{bashcode}
...
\begin{frame}[label=withcode,fragile]{Apache Spark -- ML Example}
...
\begin{frame}[label=full]{Stream Analytics}
\begin{itemize}
...
\begin{frame}[label=full]{Process Discovery with Streaming Data}
\begin{columns}
\begin{column}{.3\textwidth}
...
\begin{frame}[label=advanced]{Apache Spark -- Stream Analytics}
...
%\begin{frame}[label=full]{Apache Spark -- Stream Analytics}
...
\begin{frame}[label=withcode,fragile]{Apache Spark -- Stream Analytics}
...
\begin{frame}[label=withcode]{Apache Spark -- Stream Analytics}
...
\begin{frame}[label=advanced,fragile]{Apache Spark -- Stream Analytics}
...
\begin{frame}[label=withcode]{Apache Spark -- Further Reading}
