...
\begin{frame}[label=full]{Linear Regression}
...
\begin{frame}[label=full]{Ensure a Linear Model is Sensible}
\centering
...
\begin{frame}[label=full]{Correlation versus Regression}
\centering
...
\begin{frame}[label=full]{Estimating Linear Regression \small [cont'd]}
\begin{itemize}
...
\begin{frame}[label=full]{Hands-On Exercise -- Linear Regression with OLS}
\begin{columns}
...
Calculate the slope and intercept of the best fitting curve for the following data points:\\
\bigskip
...
\begin{frame}[label=full]{Evaluating Linear Regression Models \small [cont'd]}
%\begin{itemize}
...
\end{column}
\begin{column}{0.8\textwidth}
Using the slope and intercept of the best fitting curve from the previous exercise, determine the predicted or fitted values. Then, determine the RSS, the TSS, and the $R^2$ value.\\

\bigskip
\renewcommand{\arraystretch}{1.25}
\begin{center}
\begin{tabular}{c|c|c|c|c|c} \hline
{\bf x} & {\bf y} & $\hat{y}$ & $(y - \hat{y})^2$ & $\bar{y}$ & $(y - \bar{y})^2$ \\ \hline
1 & 2 & & & &\\
2 & 2 & & & & \\
2 & 3 & & & & \\
3 & 3 & & & & \\ 
...
\begin{frame}[label=full]{Generalization to Multiple Predictors}
\begin{columns}
...
\begin{frame}[label=full]{Regression with Qualitative Predictors}
\begin{itemize}
...
\begin{frame}[label=full]{Inputs, Predictors, and Polynomials}
...
%\begin{frame}[label=full]{Interactions}
%\centering
...
\begin{frame}[label=full]{Multiple Regression with Polynomials}
\begin{columns}
...
\begin{frame}[label=withcode,fragile]{Linear Regression in R}
\small
...
\begin{frame}[label=withcode,fragile]{Linear Regression in R \small [cont'd]}
\small
...
\begin{frame}[label=withcode]{Hands-On Exercises \small (Source: ISLR2 Chapter 3)}
\begin{columns}
...
\small
Use the \texttt{Auto} data from the ISLR2 library with \texttt{mpg} as target.
  \begin{enumerate}
...
\begin{frame}[label=withcode,fragile]{Cross-Validation in R -- Holdout Sample}
...
\begin{frame}[label=withcode,fragile]{Cross-Validation in R}
...
%\begin{frame}[label=full,fragile]{Cross-Validation in R -- K-Fold CV}
...
\begin{frame}[label=withcode]{Hands-On Exercises -- Cross-Validation}
\begin{columns}
\begin{column}{0.1\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}
\end{column}
...
\begin{enumerate}
  ...
  \item Calculate the test error estimate using LOOCV. 
  \item Calculate the test error estimate using 10-fold cross-validation. 
\end{enumerate}
\end{column}
\end{columns}
\end{frame}

\begin{frame}[label=full]{Shrinkage Methods}
\begin{block}{Goals}
\begin{itemize}
...
\begin{frame}[label=full]{Ridge Regression}{a.k.a Tikhonov Regularization}
\begin{align*}
...
\begin{frame}[label=full]{Ridge Regression Example }{Fitting a Degree 14 Polynomial}
\centering
\includegraphics[width=0.9\textwidth]{screen4.png}\\
\scriptsize Source: Murphy Figure 4.5
\end{frame}

\begin{frame}[label=full]{Lasso regression}{''Least Absolute Shrinkage and Selection Operator''}
\begin{align*}
...
\begin{frame}[label=full]{Elastic Net}
...
\begin{frame}[label=full]{Hands-On Exercise -- Shrinkage Methods}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:Paper_and_pencil_Pinhead_icon.svg}{\includegraphics[width=.75\textwidth]{../pencilblack.png}}
\end{column}
\begin{column}{0.8\textwidth}
Assuming the following polynomial regression model:
\begin{align*}
\hat{y} = 15 x^4 - 10 x^3 + 20 x^2 - 40 x + 50
\end{align*}

and a residual sum of squares (RSS) of 20, what are the values of the optimization criteria for: 
\begin{enumerate}
   \item Ridge regression with $\lambda=0.1$
   \item Lasso with $\lambda=0.1$
   \item Elastic net with $\lambda=0.1, \alpha=0.5, \beta=0.5$
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=withcode,fragile]{Ridge Regression in R}
\small
...
\begin{frame}[label=withcode,fragile]{Ridge Regression in R \small [cont'd]}
...
\begin{frame}[label=full]{Selecting the Tuning Parameter}
...
\begin{frame}[label=withcode,fragile]{Ridge Regression in R -- Optimal Lambda}
\small
...
\begin{frame}[label=withcode,fragile]{Ridge Regression in R  -- Optimal Lambda \small [cont'd]}
\small
...
\begin{frame}[label=withcode,fragile]{Ridge Regression in R -- Prediction}
\small
...
\begin{frame}[label=withcode,fragile]{The Lasso in R \small [cont'd]}
\small
...
\begin{frame}[label=withcode,fragile]{The Lasso in R -- Optimal Lambda}
\small
Show the optimal lambda, the MSE and its standard error, and the number of non-zero coefficients.
\begin{columns}
\begin{column}{.35\textwidth}
...
\begin{frame}[label=withcode,fragile]{The Lasso in R -- Prediction}
Fit test data using optimal $\lambda$:
\begin{Rcode}
...
\begin{frame}[label=withcode]{Hands-On Exercises -- Shrinkage Methods}{Source: ISLR2, Chapter 6}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}
\end{column}
...
\begin{frame}[label=advanced]{Robust Regression}
\begin{block}{Problem}
\begin{itemize}
...
\begin{frame}[label=full]{Classification}
  \begin{itemize}
...
\begin{frame}[label=full]{Logistic Regression}
...
\begin{frame}[label=full]{Hands-On Exercise -- Logistic Regression}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:Paper_and_pencil_Pinhead_icon.svg}{\includegraphics[width=.75\textwidth]{../pencilblack.png}}
\end{column}
\begin{column}{0.8\textwidth}
The slope and intercept of the best fitting curve for the following data points are $1.333$ and $0$ respectively. Calculate the predicted probability for each $x$ value.

\bigskip
\renewcommand{\arraystretch}{1.25}
\begin{center}
\begin{tabular}{c|c|c} \hline
{\bf x} & {\bf y} & {\bf p(y)}\\ \hline
-2 & -2 & \\
-1 & -2 & \\
-1 & -1 & \\
0 & -1 & \\ 
...
\begin{frame}[label=full]{Logistic Regression \small [cont'd]}
...
\begin{frame}[label=advanced]{Multinomial Logistic Regression}
\small
\begin{align*}
...
\begin{frame}[label=advanced]{Logistic Regression with Polynomials}
...
\begin{frame}[label=advanced]{Logistic Regression with Polynomials \small [cont'd]}
...
\begin{frame}[label=withcode,fragile]{Logistic Regression in R}
\small
...
\begin{frame}[label=withcode,fragile]{Logistic Regression in R -- Holdout Set} 
\small
...
\begin{frame}[label=withcode,fragile]{Logistic Regression in R -- Evaluation}
\small
...
\begin{frame}[label=advanced]{Naive Bayes Classifier}
\small \vspace{-3mm}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{Naive Bayes Classifier in R}
\small
...
\begin{frame}[label=withcode,fragile]{Naive Bayes Classifier in R -- Evaluation}
\small
...
\begin{frame}[label=withcode]{Naive Bayes Classifier in R -- Evaluation}
\centering
\includegraphics[height=2.5in]{nb_roc.pdf}
\end{frame}

\begin{frame}[label=withcode,fragile]{KNN Classification in R}
\small
...
\begin{frame}[label=withcode,fragile]{KNN Classification in R \small [cont'd]}
\small 
...
\begin{frame}[label=withcode]{Hands-On Exercises}{Source: ISLR2, Chapter 4}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}
\end{column}
...
\begin{enumerate}
   \item Create a new binary variable \texttt{crime01} that is $1$ is \texttt{crime} is above its median, and $0$ otherwise. Combine this variable with the data frame. \emph{Tip}: Use the \texttt{median()} function.
   \item Split your data set into a training and test set
