\usepackage[x11names]{xcolor}
\usepackage[most]{tcolorbox}
\usepackage{physics}
\usepackage{wrapfig}
\usepackage{pgf}
\usepackage[english]{babel}
\usepackage[latin1]{inputenc}
\usepackage{times}
\usepackage[T1]{fontenc}
\usepackage{fancyvrb}
\usepackage{multicol}
\usepackage{multirow}
\usepackage{tabularx}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{bbm}
\usepackage{alltt}
\usepackage{placeins}
\usepackage{enumitem}
\usepackage{subcaption}
\usepackage{gensymb}
\usepackage{titlepic}
\usepackage{parskip}
\usepackage{imakeidx}

% Conditional compilation with or without code
% based on BASH variable _BUSI4720_BOOK_CODE
\usepackage{catchfile}
\usepackage{xstring}
\usepackage{comment}
\immediate\write18{echo -n "$\string_BUSI4720_EDITION" > busi4720_edition_tmp.txt}
\CatchFileDef{\IncludeCode}{busi4720_edition_tmp.txt}{\endlinechar=-1}
\IfStrEqCase{\IncludeCode}{
{Full}{   
	\includecomment{withcode}
	\includecomment{advanced}
    \excludecomment{nocode}
    \excludecomment{simplified}
}
{NoCode}{
    \includecomment{nocode}
    \includecomment{advanced}
    \excludecomment{withcode}
    \excludecomment{simplified}
}
{Simplified}{
    \excludecomment{withcode}
    \excludecomment{advanced}
    \excludecomment{nocode}
    \includecomment{simplified}
}
}

\makeindex[intoc]
\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    filecolor=magenta,      
    urlcolor=blue,
}
\usepackage{subcaption}
%\usepackage{subfigure}
\usepackage{placeins}
\usepackage{minted}
%\usepackage{graphicx}
\newminted{cypher}{autogobble,label=cypher,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single}
\newminted{R}{autogobble,label=R,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single}
\newminted{xml}{autogobble,label=XML,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single}
\newminted{bash}{autogobble,label=bash,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single}
\newminted{html}{autogobble,label=HTML,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single}
\newminted{js}{autogobble,label=JavaScript,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single}
\newminted{sql}{autogobble,label=SQL,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single}
\newminted{text}{autogobble,label=Text,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single}
\newminted{json}{autogobble,label=JSON,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,samepage=true,frame=single,escapeinside=||}
\newminted{python}{autogobble,label=Python,numbers=left,stepnumber=2,bgcolor=Gray0!25!white,breakbytoken,python3,samepage=true,frame=single,style=xcode}
\AtBeginEnvironment{minted}{%
  \renewcommand{\fcolorbox}[4][]{#4} \footnotesize}
\AtEndEnvironment{minted}{%
  \normalsize}

\definecolor{code}{gray}{0.95}
\definecolor{alert}{RGB}{238,224,229} % LavenderBlush2 

%\newtcolorbox{exercisebox}{%
 %enhanced,
 %colback=Gray0!25!white,
 %colframe=Gray0!60!black,
 %title={Hands-On Exercise},
 %fonttitle=\large,
 %leftrule=10mm,
 %overlay={
   %\node[anchor=north west,outer sep=1pt] 
     %at (frame.west) 
     %{\includegraphics[width=8mm]{../hand2.png}};
  %}}

\newtcolorbox[auto counter, number within=chapter]{exerciseboxpaper}{%
  enhanced,
  title={Hands-On Exercise \thetcbcounter},
  colback=Gray0!25!white,
  colframe=Gray0!60!black,
  fonttitle=\large,
  leftrule=10mm,
  overlay={
    \node[anchor=north west,outer sep=1pt]
      at (frame.west)
      {\includegraphics[width=8mm]{../pencil.png}};
  }
}

\newtcolorbox[use counter from=exerciseboxpaper]{exerciseboxcomputer}{%
  enhanced,
  title={Hands-On Exercise \thetcbcounter},
  colback=Gray0!25!white,
  colframe=Gray0!60!black,
  fonttitle=\large,
  leftrule=10mm,
  overlay={
    \node[anchor=north west,outer sep=1pt]
      at (frame.west)
      {\includegraphics[width=8mm]{../computer.png}};
  }
}

\newtcolorbox{resourcebox}{enhanced,colback=LightSteelBlue1!25!white,colframe=LightSteelBlue1!60!black,title={Resources},fonttitle=\large,leftrule=10mm,overlay={\node[anchor=north west,outer sep=0.5pt] at (frame.west) {\includegraphics[width=8mm]{../book.png}};}}

\newtcolorbox{videobox}{enhanced,colback=Chocolate1!25!white,colframe=Chocolate1!60!black,title={Video},fonttitle=\large,leftrule=10mm,overlay={\node[anchor=north west,outer sep=0.5pt] at (frame.west) {\includegraphics[width=8mm]{../videowhite.png}};}}

\newtcolorbox{alertbox}{enhanced,colback=OrangeRed1!25!white,colframe=OrangeRed1!60!black,title=Important,fonttitle=\large,leftrule=10mm,overlay={\node[anchor=north west,outer sep=0.5pt] at (frame.west) {\includegraphics[width=8mm]{../attention2.png}};}}

\newtcolorbox{infobox}{enhanced,colback=DarkSeaGreen1!25!white,colframe=DarkSeaGreen1!60!black,title=Tip,fonttitle=\large,leftrule=10mm,overlay={\node[anchor=north west,outer sep=0.5pt] at (frame.west) {\includegraphics[width=8mm]{../tip1a.png}};}}

\newtcolorbox{pseudocodebox}{enhanced,colback=code,colframe=code!50!black}

\renewcommand\floatpagefraction{.95}
\renewcommand\topfraction{.95}
\renewcommand\bottomfraction{.95}
\renewcommand\textfraction{.05}   

\newcommand{\argmax}{\operatorname*{argmax}}
\newcommand{\argmin}{\operatorname*{argmin}}
\newcommand{\Ident}{\operatorname{I}}

\author{Joerg Evermann}
\date{}
