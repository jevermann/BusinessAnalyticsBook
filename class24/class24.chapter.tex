...
\begin{advanced}
\begin{videobox}
A video introduction to this chapter is available on YouTube:\\

\url{https://www.youtube.com/watch?v=qLnoySoLr-c}
\end{videobox}
\end{advanced}
...
\begin{exerciseboxpaper}
For each information item, indicate whether it is personal information:
\begin{minipage}[t]{0.54\textwidth}
\begin{itemize}
\item Employee ID number
\item Company annual revenue 
\item Business email address 
\item Anonymous survey results 
\item IP address 
\item Customer purchase history 
\item Weather report 
\item Vehicle license plate 
\end{itemize}
\end{minipage}
\begin{minipage}[t]{0.44\textwidth}
\begin{itemize}
\item Handwriting sample 
\item Company logo 
\item Home address 
\item Job title only 
...
\bigskip
\begin{enumerate}
    \item Which items were the most difficult to classify?
    \item Why does context affect whether information is personal?
\end{enumerate}
\end{exerciseboxpaper}
...
\begin{exerciseboxpaper}
For each scenario, determine whether consent is:\\

\begin{center}
\textbf{Valid Consent \quad Invalid Consent \quad More Info Needed}
\end{center}
\bigskip
\begin{enumerate}
\item A retailer explains that an email address will only be used for receipts. The customer voluntarily provides the address.
\item A website hides consent inside a 40-page Terms of Service agreement.
\item A bank requests Social Insurance Number information when opening an account.
\item A company uses customer emails collected for shipping notifications to send advertising without informing customers.
\item A mobile application requests access to contacts but does not explain why.
\end{enumerate}
\end{exerciseboxpaper}

\begin{exerciseboxcomputer}

Consider your bank or communications provider or other large organization you regularly deal with. Research the following questions:
\begin{itemize}
...
\begin{exerciseboxpaper}
A gym creates a membership form. Decide whether each item is: \\

\begin{center}
\textbf{Necessary} \quad \textbf{Optional} \quad \textbf{Not Appropriate}
\end{center}
...
\begin{exerciseboxpaper}
For each of the following records choose whether to: \\

\begin{center}
\textbf{Keep} \quad \textbf{Archive} \quad \textbf{Securely Destroy}
\end{center}
\bigskip
\begin{enumerate}
\item Customer who closed account 12 years ago
\item Active employee file
\item Duplicate printed report 
\item Outdated mailing list 
\item Tax records 
\item Expired job applicant resumes 
\item Temporary meeting notes 
\end{enumerate}
\end{exerciseboxpaper}
...
A customer states:\\
\begin{quote}
"I want to know everything your company has about me."
\end{quote}
\bigskip
As Privacy Officer you must answer these questions:
\bigskip
\begin{itemize}
    \item What information should be searched?
    \item What timelines apply?
    \item What information may be withheld?
    \item How should the response be communicated?
\end{itemize}
\end{exerciseboxpaper}
...
\begin{exerciseboxpaper}
A marketing company stores customer files in unlocked filing cabinets. Employees share passwords. Old customer records from ten years ago are never deleted. Staff discuss customer complaints in public areas. Visitors can see computer screens displaying customer information. \\

\begin{enumerate}
\item Identify 5 privacy issues and the PIPEDA principles that are involved.
\item Briefly describe recommendation for improvements for each issue.
\end{enumerate}
\end{exerciseboxpaper}
...
This section examines the proposed Artificial Intelligence and Data Act (AIDA). This act was part of the Digital Charter Implementation Act 2022 which combined the Consumer Privacy Protection Act (an update to PIPEDA), the Personal Information and Data Protection Tribunal Act, and the Artificial Intelligence and Data Act (AIDA)\index{Artificial Intelligence and Data Act}\index{AIDA!see{Artificial Intelligence and Data Act}}. 

\begin{infobox}
The Digital Charter Act was introduced in the House of Commons of the Parliament of Canada as bill C-27\index{Bill C-27}, sponsored by the Minister of Innovation, Science, and Industry, in June 2022. It received a second reading in April of 2023. With the resignation of prime minister Justin Trudeau and prorogation of parliament, this bill died on the order of papers. \end{infobox}
...
While the bill died on the order of papers, it is still instructive to examine the proposed legislation. It shows the increasing awareness of governments of the potential risks presented by AI systems and the willingness of governments to regulate the development, use and offering of such systems. The intent of the proposed legislation was to prohibit reckless and malicious use of AI systems, and to ensure accountability of risks associated with the use of AI systems. The act defined AI as:
...
However, the proposed AIDA applied only to ''high-impact systems''. When considering whether an AI system has a high impact, the government would have considered factors such as the potential to inflict serious harm, whether intended or unintended, the scale of use of the system, whether there is evidence of risk to health or safety or of a negative impact on human rights (e.g. discrimination or differentiation based on prohibited factors, such as ethnicity, gender, etc.), whether harm or adverse impact has already occured, whether there is an imbalance of economic or social circumstances, and whether the risks are adequately regulated under another law. 

Examples of high-impact systems are screening systems that make decisions, recommendations, or predictions that affect an individual's access to services, benefits, or employment (e.g. credit scoring systems). These systems were argued to pose risk due to the potential for discriminatory outcomes and economic harm to individuals. 

Another example of high-impact systems are biometric systems that identify people remotely in order to make predictions about their behaviour, their characteristics, or their psychology. These were argued to have a potential negative impact on mental health and autonomy. 

Systems that influence behaviour at scale, such as content recommendation systems found on social media platforms, with their potential impacts on psychological and physical health would have also been considered high-impact AI systems. 

AI systems that are integrated in health and safety functions or in critical infrastructure might also have been classified as high-impact AI systems. Examples are autonomous driving systems and triage decision making systems in health care settings. Such systems were argued to have the potential to cause physical harm. 

AIDA was concerned with individual harms, collective harms (e.g. human rights impacts, or impacts on historically marginalized communities) as well as biased output. The proposed act defined biased output as follows:
...
AIDA would have required organizations to implement measures to identify, evaluate, and mitigate or reduce the risk of harm or biased output. Specifically, AIDA's requirements were guided by following six principles.

\emph{Human Oversight and Monitoring:} This principle would have required organizations to exercise meaningful human oversight over decisions or recommendations made by an AI system. It may have required a ''human-in-the-loop'' when operating such systems, and it would have required systems to be designed to allow such oversight. Effective human oversight would have required the behaviour of the AI system to be interpretable, with the specific level of interpretability dependent on the context and purpose of the system. Monitoring of input and output (predictions, recommendations, decisions) of an AI system would have been required so that human oversight could have been performed after the fact. 

\emph{Transparency:} This principle would have required organizations to provide information to individuals and the regulator about how a high-impact AI system is used, and what its capabilities, limitations, and potential impacts are. 

\emph{Fairness and Equity:} This principle would have required organizations to demonstrate awareness of potential discriminatory outcomes and to have taken actions to mitigate such outcomes.

\emph{Safety:} The safety principle would have required organizations to pro-actively evaluate potential harms stemming from the development or use of a high-impact AI system and to have taken measures to reduce the risk of harm.

\emph{Accountability:} The accountability principle would have required organizations to implement governance mechanisms to ensure compliance with AIDA. This would have taken the form of documentation of policies, processes, and any measures for risk reduction, bias reduction, and safety improvements. 

\emph{Validity and Robustness:} This principle would have required organizations to ensure that high-impact AI systems operate in a valid way, that is, consistent with the intended objectives, and reliably, that is, they are resilient and stable in a variety of different circumstances. 

\subsection*{Regulated Activities under AIDA}

AIDA would have regulated four types of activities throughout the life-cycle of a high-impact AI system.

\emph{System Design:} Organizations that design high-impact AI systems would have had to take measures to identify and reduce risks and bias during system design, and to document appropriate use and the limitations of the system. Specific examples are risk assessment during initial system design, during training data set selection, and when determining the level of interpretability of the output that is required and provided by the system.

\emph{System Development:} During development of a high-impact AI system, including its training, organizations would have been required to document the training data and the models themselves. They would have needed to evaluate the performance of the model in different situations and to retrain the model as needed in order to minimize risk and bias of the output. Organizations would have also been required to build mechanisms to allow human oversight and monitoring.

\emph{Make System Available for Use:} When making a high-impact AI system available for use, organizations would have been required to document how the system meets its requirements for a safe and unbiased design. They would also have had to provide documentation to users of the system with information about the data sets that were used for training, information on the limitations of the system, and on the appropriate uses of the system. Organizations would have been required to continuously review their risk assessment as the system is operated.

\emph{Manage Operations of a System}: Organizations that manage the operation of a high-impact AI system would have been required to maintain records of the inputs and outputs of a system and monitor its performance. They would have had to ensure adequate monitoring and human oversight over recommendations and decisions made by the system. Organizations would have been required to intervene in the operation of a system if its behaviour fell outside of established operational parameters or expected performance parameters. 

\subsection*{Enforcement of AIDA}

The proposed act included the establishment of an AI and Data Commissioner, analogous to the role of the Privacy Commissioner established by PIPEDA. Also analogous to PIPEDA, AIDA contained a notification requirement in case of harm of potential material harm. However, in contrast to PIPEDA, the Minister of Industry, Science, and Economic Development would have had enforcement powers in the form of the right to request demonstration of compliance, the right to order an independent audit, the ability to levy administrative monetary penalties, and the right to stop the use of a high-impact AI system. 

Additionally, the proposed act would have established three new criminal offences\footnote{\href{https://www.parl.ca/DocumentViewer/en/44-1/bill/C-27/first-reading}{Bill C-27, House of Commons}}. Note that the first of the following proposed offences would also have applied to data collected by third parties outside of Canada.
\begin{itemize}
...
The proposed act left concrete details unspecified, to be defined through subsequent regulation after the act had received royal assent (that is, after it became law). 
