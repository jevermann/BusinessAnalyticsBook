...
\begin{frame}[label=withcode]{Resources}
...
\begin{frame}[label=full]{MLOps Purpose}
\begin{itemize}
...
\begin{frame}[label=full]{ML Challenges}
...
\begin{frame}[label=full]{MLOps Principles}
\begin{enumerate}
...
\begin{frame}[label=full]{MLOps -- Relationship to other disciplines}
\begin{columns}
...
\begin{frame}[label=full]{Model Development Lifecycle}
\includegraphics[width=\textwidth]{mlmodel.jpeg}
\end{frame}

\begin{frame}[label=full]{Software Development Lifecycle}
\includegraphics[width=\textwidth]{devopsmodel.jpeg}
\end{frame}

%\begin{frame}[label=full]{ML Lifecycle}
%\includegraphics[width=\textwidth]{mlops.jpeg}
%\end{frame}

\begin{frame}[label=full]{MLOps Lifecycle}
\includegraphics[width=\textwidth]{mlops2.jpeg}
\end{frame}


\begin{frame}[label=full]{MLOps Participants}
\begin{itemize}
...
\begin{frame}[label=full]{MLOps Participants -- Overlapping Roles}
\begin{columns}
\begin{column}{.9\textwidth}
\centering
...
\begin{frame}[label=full]{MLOps Participants -- Roles in MLOps lifecycle}
\begin{columns}
...
\begin{frame}[label=full]{MLOps -- Requirements}
\begin{itemize}
   \item Subject matter experts:
   \begin{itemize}
  \footnotesize
...
\begin{frame}[label=full]{MLOps Requirements \small [cont'd]}
\begin{itemize}
   \item Model risk managers and model auditors:
   \begin{itemize}
  \footnotesize
...
\small
For each scenario, identify the \textbf{primary stage of the MLOps lifecycle}. Choose from:

\begin{itemize}
    \item Develop Models
    \item Prepare for Production
    \item Deploy to Production
    \item Monitoring \& Feedback
    \item Governance
\end{itemize}
\end{column}
\end{columns}
...
\begin{enumerate}
\item A data scientist evaluates several models using historical data.
\item Engineers deploy a model to 10\% of users before a full rollout. 
\item Customer behaviour changes and model accuracy decreases. 
\item The legal department reviews fairness and explainability requirements. 
\item The operations team configures health checks and failover.
\end{enumerate}
\end{frame}
...
\begin{column}{0.8\textwidth}
\small
Match each responsibility with the most appropriate MLOps participant.
\end{column}
\end{columns}
...
{\bf Responsibilities}
\begin{itemize}
   \item Defines business KPIs for the mode
   \item Builds data pipelines
   \item Integrates the model into a web application
   \item Designs automated deployment pipelines
   \item Evaluates fairness and regulatory compliance 
   \item Develops predictive models
   \item Ensures infrastructure can scale with demand 
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=full]{MLOps -- Tooling}
\begin{itemize}
...
\begin{frame}[label=full]{MLOps -- Tooling \small [cont'd]}
\begin{itemize}
  \item Model Registry
  \begin{itemize}
  \footnotesize
...
\begin{block}{Scenario}
Your organization needs to manage and track different versions of your trained machine learning models, including their metadata, evaluation metrics, and training history.
\end{block}
...
\begin{frame}[label=full]{Develop Models}
\centering
...
\begin{frame}[label=full]{Develop Models \small [cont'd]}
\begin{block}{Data}
\begin{itemize}
...
\begin{frame}[label=full]{Prepare for Production}
\centering
...
\begin{frame}[label=full]{Prepare for Production \small [cont'd]}
...
\begin{column}{0.8\textwidth}
\small
A hospital plans to deploy an ML model that predicts whether
patients should receive additional medical screening. 
For each risk, write one appropriate mitigation strategy.
\end{column}
\end{columns}
\bigskip
\begin{columns}
...
{\bf Risks:}
\begin{itemize}
\item Training data no longer represents current patients
\item Model unexpectedly stops responding 
\item Incorrect predictions affect many patients
\item An attacker attempts to manipulate model inputs
\end{itemize}
\end{column}
\begin{column}{.4\textwidth}
\small
{\bf Possible mitigations:}
\begin{itemize}
    \item Shadow testing
    \item Progressive rollout
    \item Continuous monitoring
    \item Periodic retraining
    \item Failover to a simpler model
...
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}\end{column}
\begin{column}{0.8\textwidth}
\small
...
\begin{block}{Question}
Using internet resources, research the ethical risks and challenges associated with personalized recommendation systems in e-commerce. Based on your findings, identify and briefly describe two distinct ethical risks that this Canadian retailer should consider regarding its new recommendation model.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=full]{Deploy to Production}
\centering
...
\begin{frame}[label=full]{Deploy to Production \small [cont'd]}
...
\end{column}
\begin{column}{0.8\textwidth}
The following steps of the CI/CD pipeline are out of order.

\bigskip

Number them from \textbf{1 (first)} to \textbf{8 (last)}.

\bigskip
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|p{6cm}|c|}
\hline
\textbf{Pipeline Step} & \textbf{Order} \\
\hline
...
\hline
Evaluate bias and interpretability & \\
\hline
\end{tabular}
\end{column}
\end{columns}
...
%\begin{frame}[label=full]{Detailed ML Activities}
%\begin{columns}
%\begin{column}{.9\textwidth}
%\centering
...
\begin{frame}[label=withcode]{Example Deployment Options}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{Flask Deployment Demo}
...
\begin{frame}[label=withcode,fragile]{Flask Example -- Step 1: Create a Trained Model} 
...
\begin{frame}[label=withcode,fragile]{Flask Example \small [cont'd]}
Define the model:
\begin{pythoncode}
...
\begin{frame}[label=withcode,fragile]{Flask Example -- Step 2: Serve the Model}
...
\begin{frame}[label=withcode,fragile]{Flask Example -- Step 3: Access the Service}
...
\begin{frame}[label=withcode,fragile]{Hands-On Exercise}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}
\end{column}
...
\begin{frame}[label=withcode,fragile]{Flask Example -- Web Forms}
...
\begin{frame}[label=withcode,fragile]{Tensorflow JS Example}
...
\end{htmlcode}
\end{frame}
\begin{frame}[label=withcode,fragile]{Tensorflow JS Examples \small [cont'd]}
\begin{htmlcode}
...
\begin{frame}[label=full]{Monitoring and Feedback}
\centering
...
\begin{frame}[label=full]{Monitoring and Feedback \small [cont'd]}
...
\begin{frame}[label=withcode,fragile]{Basic Python Logging}
...
\begin{frame}[label=withcode,fragile]{Basic Python Logging \small [cont'd]}
Use the logger:
\begin{pythoncode}
@app.route("/predict_json", methods=["POST"])
def predict_json():
...
\begin{frame}[label=withcode]{Hands-On Exercise}
\begin{columns}
\begin{column}{0.2\textwidth}
\href{https://commons.wikimedia.org/wiki/File:NotoSans_-_Keyboard_And_Mouse_-_1F5A6.svg}{\includegraphics[width=\textwidth]{../computerblack.png}}
\end{column}
...
\begin{frame}[label=full]{MLOps Governance}
\centering
...
%\begin{frame}[label=full]{MLOps Governance \small [cont'd]}
%\small
...
\begin{column}{0.8\textwidth}
\small
A city government plans to deploy an AI system to prioritize
road maintenance projects.

\bigskip

For each statement, answer: ''YES'', ''NO'', or ''NEED MORE INFORMATION''
\end{column}
\end{columns}
...
\begin{enumerate}
   \item Are citizens affected by the model's decisions? 
   \item Should the decisions be explainable? 
   \item Is bias testing appropriate? 
   \item Should predictions be monitored after deployment? 
   \item Should the organization document the training data?
   \item Is governance unnecessary because the model only makes recommendations? 
\end{enumerate}
\end{frame}
