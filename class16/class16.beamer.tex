...
\begin{frame}[label=full]{Convolutional Neural Networks}
\begin{block}{Task}
\begin{itemize}
...
\begin{frame}[label=full]{Convolution}
...
\begin{frame}[label=full]{Convolution \small [cont'd]}
...
\begin{frame}[label=full]{CNN Parameters}
\small
\begin{itemize}
...
\begin{frame}[label=full]{Convolution \small [cont'd]}{Striding and Padding}
...
\begin{bmatrix}  1 & 2 & 1 \\ 
                ...
                1 & 2 & 1 \end{bmatrix}
\end{align*}
\item Zero-pad the matrix with \emph{two} zeros on all sides
\item Using stride 2, calculate the convolution result. 
\item What are the dimensions of the output?
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=full]{Pooling}
\begin{itemize}
...
\small
\begin{enumerate}
\item Assume the following $4 \times 4$ input matrix:
\begin{align*}
\begin{bmatrix} 1 & 8 & 8 & 1 \\ 
                8 & 36 & 31 & 8 \\ 
                8 & 31 & 32 & 7 \\
                1 & 9 & 7 & 3
\end{bmatrix}
\end{align*}
\item Apply a $2 \times 2$ max pooling layer. 
\item What are the dimensions of the pooling output?
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=full]{CNN -- Feature Maps}
\begin{columns}
...
\begin{frame}[label=withcode]{CNN in Keras}
\centering
...
\begin{frame}[label=withcode,fragile]{CNN in Keras}
Import the data:
\begin{pythoncode}
...
\begin{frame}[label=full]{Other Computer Vision Tasks for CNNs}{Object Detection}
...
\begin{frame}[label=full]{Other Computer Vision Tasks for CNNs}{Semantic Segmentation}
...
\begin{frame}[label=full]{Other Computer Vision Tasks for CNNs}
\begin{center}
...
\begin{frame}[label=withcode,fragile]{Text Classification using CNN}
Import packages:
\begin{pythoncode}
import collections
import pathlib
...
\begin{frame}[label=withcode,fragile]{Text Classification using NN \small [cont'd]}
...
\begin{frame}[label=full,fragile]{Text Preprocessing}
...
\begin{frame}[label=withcode]{Hands-On Exercises}\begin{columns}
\begin{column}{0.2\textwidth}
...
\begin{frame}[label=withcode,fragile]{Text Classification Using CNN \small [cont'd]}
...
\begin{frame}[label=full]{Word Embeddings}
\begin{columns}
