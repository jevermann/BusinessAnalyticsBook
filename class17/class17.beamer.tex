...
\begin{frame}[label=withcode]{Based On}
...
\begin{frame}[label=full]{Predictions from Sequences}
...
\begin{frame}[label=full]{RNN -- Seq2Vec}
\begin{itemize}
...
\begin{frame}[label=full]{RNN -- Vec2Seq}
\begin{itemize}
...
\begin{frame}[label=full]{RNN -- Seq2Seq}
\begin{itemize}
...
\begin{frame}[label=full]{RNN -- Seq2Seq (Non-Aligned)}
...
\begin{frame}[label=full]{Recurrent Neural Networks (RNN)}
\begin{itemize}
...
\begin{frame}[label=full]{Backpropagation Through Time}
\begin{align*}
...
\begin{frame}[label=full]{Long-Short-Term Memory (LSTM) Cells}
\begin{itemize}
...
\begin{frame}[label=full]{LSTM Cells}
\begin{align*}
...
\begin{frame}[label=full]{LSTM Cell Parameters}
\small
\begin{itemize}
...
\begin{frame}[label=full]{Hands-On Exercise --- Simple LSTM}
\begin{columns}
...
\small
\begin{itemize}
  \item Assume input $x_t = 1$, prior state $h_{t-1}=0.5$ and cell memory $c_{t-1} = 0.7$
  \item Assume weights and biases as follows:
  \begin{itemize}
     \item {\bf Forget gate:} $W_f = [0.5, 0.2], \quad b_f = 0.1$
     \item {\bf Input gate:} $W_i = [0.4, 0.3], \quad b_i = 0.2$
     \item {\bf Output gate:} $W_o = [0.6, 0.1], \quad b_o = 0$
     \item {\bf Candidate cell state:} $W_c = [0.7, 0.4], \quad b_c = 0$
  \end{itemize}
\end{itemize}
\begin{enumerate}
  \item Calculate $F_t$ \hspace{2cm} {\bf Tip}: $\sigma(a)=\frac{1}{1+e^{-a}}$
  \item Calculate $I_t$
  \item Calculate $\tilde{c}_t$
  \item Calculate $c_t$
  \item Calculate $O_t$
  \item Calculate $h_t$
\end{enumerate}
...
\begin{frame}[label=full]{Gated Recurrent Unit (GRU) Cells}
\begin{itemize}
...
\begin{frame}[label=full]{GRU Cells}
\begin{align*}
...
\begin{frame}[label=full]{GRU Cell Parameters}
\small
\begin{itemize}
...
\begin{frame}[label=full]{Hands-On Exercise --- Simple GRU}
\begin{columns}
\begin{column}{0.1\textwidth}
\href{https://commons.wikimedia.org/wiki/File:Paper_and_pencil_Pinhead_icon.svg}{\includegraphics[width=.75\textwidth]{../pencilblack.png}}
\end{column}
...
\small
\begin{itemize}
  \item Assume input $x_t = 1$ and prior state $h_{t-1}=0.5$
  \item Assume weights and biases as follows:
  \begin{itemize}
     \item {\bf Update gate:} $W_z = [0.4, 0.2], \quad b_z = 0.1$
     \item {\bf Reset gate:} $W_r = [0.3, 0.4], \quad b_r = 0$
     \item {\bf Candidate cell state:} $W_h = [0.5, 0.3], \quad b_h = 0$
  \end{itemize}
\end{itemize}
\begin{enumerate}
  \item Calculate $Z_t$ \hspace{2cm} {\bf Tip}: $\sigma(a)=\frac{1}{1+e^{-a}}$
  \item Calculate $R_t$
  \item Calculate $\hat{h}_t$
  \item Calculate $h_t$
\end{enumerate}
\end{column}
\end{columns}
...
\begin{frame}[label=full]{Statefulness of RNN}
\small
...
\begin{frame}[label=withcode,fragile]{Stock Market Prediction}
...
\begin{frame}[label=withcode,fragile]{Stock Market Prediction \small [cont'd]}
...
\begin{frame}[label=withcode,fragile]{Interlude -- Dataset Objects}
...
\begin{frame}[label=withcode]{Hands-On Exercise -- Dataset Objects}
\begin{columns}
\begin{column}{0.2\textwidth}
...
\begin{frame}[label=withcode,fragile]{Next Activity Prediction of a Business Process}
\small
...
\begin{frame}[label=withcode,fragile]{Next Activity Prediction of a Business Process \small [cont'd]}
Fix the data types:
\begin{pythoncode}
log['time:timestamp'] = \
...
%\begin{frame}[label=full,fragile]{Next Activity Prediction of a Business Process \small [cont'd]}
