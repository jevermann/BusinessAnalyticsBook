...
Consider an LSTM cell with hidden state $h_t$ and cell memory $c_t$ sizes of $n=16$ and an input size $x_t$ of $m=10$. This means that there must be $m+n=26$ weights for the forget gate $W_f$ in equation \ref{eq:forget}. That is, $W_f$ is a $1 \times (m+n)$ row vector that is multiplied with the $(m+n) \times 1$ column vector $[x_t, h_{t-1}]$ to yield a single number (a $1 \times 1$ ''vector''). The bias term is then added and the sigmoid function is applied to this sum. The total number of parameters is then of $m+n+1=27$.
