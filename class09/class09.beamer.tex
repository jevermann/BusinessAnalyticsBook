...
\begin{frame}[label=full]{Business Processes}
\begin{itemize}
...
\begin{frame}[label=full]{BPMN Example Process Model}
\centering
\includegraphics[width=1.15\textwidth]{Figure3.12.pdf}
\end{frame}

\begin{frame}[label=full]{Business Process Instances and Process Data}
\begin{block}{Case}
\begin{itemize}
...
\begin{frame}[label=full]{Activity Lifecycle -- Example}
\centering
...
\begin{frame}[label=full]{Event Log Data}
\begin{block}{Sources}
\begin{itemize}
...
\begin{frame}[label=full]{Process Analytics}
\centering
...
\begin{frame}[label=withcode]{Process Analytics in R}
\begin{block}{BupaR}
\begin{itemize}
...
\begin{frame}[label=withcode]{Process Analytics in Python}
\begin{block}{PM4PY}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{PM4PY Basics}
Read a CSV dataset:
\footnotesize
\begin{pythoncode}
import pandas as pd
...
\begin{frame}[label=withcode,fragile]{PM4PY Basic Statistics}
\footnotesize
\begin{pythoncode}
...
%\begin{frame}[label=full,fragile]{PM4PY Statistics on Dataframes}
%\footnotesize
%\begin{pythoncode}
...
\begin{frame}[label=withcode,fragile]{Process Discovery -- DFG}
...
\begin{frame}[label=full]{Process Discovery -- DFG}
\centering
...
Draw a DFG graph for the following traces in an event log: \\ \vspace{\baselineskip}
...
\begin{enumerate}
   ...
   \item Create and visualize a directly-follows-graph
...
\begin{frame}[label=full]{Process Discovery -- Inductive Miner}
...
\begin{frame}[label=withcode,fragile]{Process Discovery -- Inductive Miner}
\footnotesize
\begin{pythoncode}
bpmn_model = \
...
\begin{frame}[label=full]{Process Discovery -- Heuristics Net}
\begin{block}{Principles}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{Process Discovery -- Heuristics Net}
\footnotesize
\begin{pythoncode}
...
\begin{frame}[label=full]{Process Discovery}
...
\begin{frame}[label=advanced]{Fitness \& Precision}
...
\begin{frame}[label=withcode,fragile]{Fitness}
\footnotesize
\begin{pythoncode}
petri_net, initial_marking, final_marking = \
...
\begin{frame}[label=withcode,fragile]{Precision}
\footnotesize
\begin{pythoncode}
...
\begin{frame}[label=full]{Log Filtering}
\begin{itemize}
...
\begin{frame}[label=withcode]{Example Filters}
\scriptsize
...
\begin{enumerate}
    ...
    \begin{itemize}
       \item \href{https://processintelligence.solutions/static/api/2.7.11/generated/pm4py.stats.get_all_case_durations.html}{\texttt{pm4py.stats.get\_all\_case\_durations}}
       \item Put case durations into a \texttt{pd.DataFrame}
       \item 1 day = 86400 seconds
       \item Use \href{https://plotly.com/python/histograms/}{\texttt{px.histogram}} 
    \end{itemize}
\end{enumerate}
\end{column}
\end{columns}
...
\begin{frame}[label=withcode,fragile]{Performance Mining --- DFG}
\begin{itemize}
...
\begin{frame}[label=full,fragile]{Performance Mining --- Annotated DFG}
\centering
...
\begin{frame}[label=full]{Performance Mining --- Dotted Chart}
\begin{itemize}
...
\begin{withcode}
\begin{frame}[label=withcode,fragile]{Performance Mining --- Dotted Chart}
\footnotesize
\begin{pythoncode}
...
\begin{frame}[label=withcode,fragile]{Performance Mining --- Performance Spectrum}
\begin{itemize}
...
\begin{frame}[label=withcode]{Performance Mining --- Performance Spectrum}
\includegraphics[width=\textwidth]{perfspectrum.png}
\end{frame}

\begin{frame}[label=withcode,fragile]{Performance Mining --- Event Distribution}
\begin{itemize}
...
\begin{frame}[label=withcode]{Performance Mining --- Event Distribution}
\centering
\includegraphics[width=.8\textwidth]{eventsdistribution.png}
\end{frame}

\begin{frame}[label=withcode,fragile]{Performance Mining --- Events per Time}
\footnotesize
\begin{pythoncode}
...
\begin{frame}[label=withcode]{Performance Mining --- Events per Time}
\centering
\includegraphics[width=.8\textwidth]{eventspertime.png}
\end{frame}


\begin{frame}[label=withcode,fragile]{Organizational Mining}
...
\begin{frame}[label=full]{Organizational Mining \\ \small Activity-Based Resource Similarity}
\centering
...
\begin{frame}[label=full]{Organizational Mining\\ \small Handover of Work Network}
\centering
...
\begin{frame}[label=full]{Oganizational Mining \\ \small Working-Together Network}
\centering
