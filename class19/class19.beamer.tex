...
\begin{frame}[label=full]{Additional Materials}
\small
...
\begin{frame}[label=withcode,fragile]{Tools}
...
\begin{frame}[label=full]{Importance of Interpretability}
...
\begin{frame}[label=full]{Model Interpretability}
...
%\begin{frame}[label=full]{Scope of Interpretability}
%\small
%\begin{itemize}
...
%\begin{frame}[label=full]{Evaluating Interpretability}
%\begin{itemize}
...
%\begin{frame}[label=full]{Properties of Explanations}
...
%\begin{frame}[label=full]{Comprehensible Explanations}
...
\begin{frame}[label=full]{Intrinsically Interpretable Models}
...
\begin{frame}[label=withcode,fragile]{Linear Regression}
Using R:
\begin{Rcode}
...
%\begin{frame}[label=full,fragile]{Linear Regression}
%\begin{Rcode}
%> bike$cnt[6]
%[1] 1606
%> bike$temp[6]
...
\begin{frame}[label=full]{Decision Trees}
...
\begin{frame}[label=full]{Regression Trees}
\begin{columns}
...
\begin{enumerate}
   ...
   \vspace{\baselineskip}
       ...
       \end{align*}
       \item Choose $j$ and $s$ to minimize total variance:
       \begin{align*}
...
\begin{frame}[label=full]{Hands-On Exercise --- Regression Trees}
\begin{columns}
...
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{c | c | c }
\textbf{ID} & \textbf{Customer Spend} & \textbf{Customer Value} \\
\hline
A & 1 & 50 \\
B & 2 & 65 \\
C & 3 & 65 \\
...
\vspace{\baselineskip}
Build a regression tree by successively splitting the regions based on the Sum of Variance criterion. At every step:
\begin{enumerate}
   \item Identify possible split points
   \item Calculate the variance for each candidate region and their sum
   \item Identify the split with the minimum total variance
   \item Repeat to a depth of two, that is, the root split and one more for each region
   \item What is the prediction for each leaf node?
\end{enumerate}
\end{frame}

\begin{frame}[label=withcode,fragile]{Regression Trees -- Python}
Prepare data:
\begin{pythoncode}
...
\begin{frame}[label=full,fragile]{Regression Trees -- Python}
...
%\begin{frame}[label=full]{Regression Trees -- Python}
%\centering
%%\hspace{-.5in}
...
\begin{frame}[label=full]{Regression Trees -- Fitted vs Actual Values}
...
\begin{frame}[label=withcode,fragile]{Hands-On Exercises}
\begin{columns}
\begin{column}{0.1\textwidth}
...
\end{itemize}
\item Split the data into a test and training data set, like:
\begin{pythoncode}
...
%\begin{frame}[label=full]{Regression Trees -- Pruning}
...
%\begin{frame}[label=full,fragile]{Regression Trees -- Pruning}
...
%\begin{frame}[label=full]{Regression Tree -- Pruning}
%MSE versus $\alpha$: \\
%\begin{center}
...
%\begin{frame}[label=full,fragile]{Regression Trees}
...
\begin{frame}[label=full]{Classification Trees}
\begin{itemize}
...
\begin{frame}[label=advanced]{Classification Trees \small [cont'd]}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{Classification Trees \small [cont'd]}
...
\begin{frame}[label=advanced]{Tree Ensemble Methods}
...
\begin{frame}[label=advanced]{Tree Ensemble Methods \small [cont'd]}
...
\begin{frame}[label=full]{Tree Ensemble Methods \small [cont'd]}
...
\begin{frame}[label=advanced,fragile]{Tree Ensemble Methods \small [cont'd]}
Bagging in Python:
\begin{pythoncode}
...
\begin{frame}[label=withcode,fragile]{Tree Ensemble Methods \small [cont'd]}
...
\begin{frame}[label=withcode]{Decision Trees}
...
\begin{frame}[label=full]{Global Model Agnostic Methods}
\begin{itemize}
...
\begin{frame}[label=full,fragile]{Partial Dependence Plot (PDP)}
...
\begin{frame}[label=withcode,fragile]{Partial Dependence Plot (PDP)}
Read the data set:
\begin{pythoncode}
import pandas as pd
d=pd.read_csv('https://evermann.ca/busi4720/bike.csv')
...
\begin{frame}[label=full]{Partial Dependence Plot (PDP)}
\begin{center}
\includegraphics[width=\textwidth]{pdp_dtr.png}
\end{center}
\end{frame}

\begin{frame}[label=full,fragile]{Individual Conditional Expectation (ICE) Plot}
\begin{itemize}
...
\begin{frame}[label=full,fragile]{Accumulated Local Effects (ALE) Plot}
\begin{itemize}
...
\begin{frame}[label=full]{ALE Plots}
\centering
...
\begin{frame}[label=withcode,fragile]{Accumulated Local Effects (ALE) Plot}
Train model:
\begin{pythoncode}
from sklearn.tree import DecisionTreeRegressor
regr=DecisionTreeRegressor(min_samples_leaf=10).fit(x,y)
...
\begin{frame}[label=full]{Accumulated Local Effects (ALE) Plot}
\centering 
...
\begin{frame}[label=full]{Permutation Feature Importance}
...
\begin{frame}[label=withcode,fragile]{Permutation Feature Importance}
Prepare data:
\begin{pythoncode}
import pandas as pd
d=pd.read_csv('https://evermann.ca/busi4720/bike.csv')
...
\begin{frame}[label=full,fragile]{Global Surrogate Models}
\begin{block}{Intuition}
...
\begin{frame}[label=full]{Global Model Agnostic Methods -- Summary}
\footnotesize
...
\begin{frame}[label=full]{Local Interpretable Model-Agnostic Explanations}
\small
\begin{block}{Idea}
\begin{enumerate}
...
\begin{frame}[label=full]{LIME -- Example}
\begin{columns}
\begin{column}{.8\textwidth}
...
\begin{frame}[label=advanced]{LIME -- Example}
\begin{itemize}
...
\begin{frame}[label=withcode,fragile]{LIME -- Example}
...
\begin{frame}[label=full]{LIME for Images}
...
\begin{frame}[label=full]{Shapley Values}
...
\begin{frame}[label=full]{Shapley Value}
...
\begin{frame}[label=full]{Hands-On Exercise --- Shapley Values}
\begin{columns}
\begin{column}{0.1\textwidth}
...
\begin{column}{0.9\textwidth}
\footnotesize
A company has three departments Marketing (A), Engineering (B), Design (C) that can work together on a project:
\begin{align*}
N=\{A,B,C\}
\end{align*}
The value created by each possible coalition is given by:
\begin{columns}
...
\begin{enumerate}
    \item Compute the Shapley values $\phi_A,\phi_B,\phi_C $ for each department:
	...
	Write out the set $S$ and the summand for each coalition $S$ in the sum!
    \item Verify that the Shapley values satisfy the efficiency property:
\begin{align*}
...
\begin{frame}[label=full]{Shapley Values in Interpretable ML}
\begin{itemize}
...
\begin{frame}[label=full]{Shapley Additive Explanations (SHAP)}
\small
\begin{columns}
...
\begin{frame}[label=withcode,fragile]{SHAP Example}
...
\begin{frame}[label=full,fragile]{SHAP Example}
...
\begin{frame}[label=full]{SHAP for Image Classification}
\begin{itemize}
...
\begin{frame}[label=full]{SHAP for Text Classification}
...
\begin{frame}[label=withcode,fragile]{Fixes for Matplotlib}
\begin{bashcode}
